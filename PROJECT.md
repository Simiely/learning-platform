...
learning-platform/
├── apps/core/              # 核心应用
│   ├── models.py           # Category, Item, LearningProgress, QuizResult
│   ├── views.py            # 所有视图 + API
│   ├── image_utils.py      # emoji取色 + 图片焦点检测
│   └── management/commands/
│       ├── seed_data.py    # 种子数据（21只动物），含手动校准的 image_position
...
├── docker-compose.yml
├── .github/workflows/build.yml  # CI：test → build & push
├── README.md               # 用户文档
├── DEV.md                  # 开发笔记（踩坑记录）
└── PROJECT.md              # 这个文件
```
