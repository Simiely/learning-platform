...
| 文件 | 作用 |
|------|------|
| **[README.md](./README.md)** | 项目介绍、功能特性、快速开始、部署说明（本文件） |
| **[DEV.md](./DEV.md)** | **开发笔记** — 记录开发过程中遇到的关键问题与解决方案。修改项目前必读，避免重复踩坑 |
| **[ANIMALS.md](./ANIMALS.md)** | **动物数据清单** — 41 只动物的完整数据（焦点、科普等），修改动物信息时参考 |
| **[PROJECT.md](./PROJECT.md)** | **项目上下文** — 给 AI 或接手者看的项目决策记录、架构说明 |
| **[HANDOVER.md](./HANDOVER.md)** | **交接文档** — 项目基本信息、环境配置、部署信息汇总 |
| **[ADD_ANIMALS_GUIDE.md](../ADD_ANIMALS_GUIDE.md)** | **增加动物指南** — 新增动物的完整操作流程（含图片素材、音频生成、代码修改） |

> 提示：首次接触项目建议先读 `DEV.md` 和 `PROJECT.md`，了解关键决策和踩坑记录后再开始修改。
...
完整动物数据、图片焦点、科普知识详见 **[ANIMALS.md](./ANIMALS.md)**。目前包含 **41 种动物**（分 4 组浏览），每只配有：
