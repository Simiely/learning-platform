...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        update_fields_map = {
            ...
            'image_position_ipad_portrait': 'image_position_ipad_portrait',
        ...
        for a in all_items:
            try:
                ...
                missing.append(f"{a.name}({a.code})")
                continue

            changed_fields = []
            for field_name in update_fields_map:
                seed_val = getattr(a, field_name)
            ...
            if changed_fields:
                ...
                for field, old, new in changed_fields:
                    self.stdout.write(
                        self.style.SUCCESS(f"  {a.name} {field}: {old} -> {new}")
                    )
                updated += 1
